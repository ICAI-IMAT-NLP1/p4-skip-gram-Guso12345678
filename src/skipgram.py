...
class SkipGramNeg(nn.Module):
    ...
    def forward_input(self, input_words: torch.Tensor) -> torch.Tensor:
        ...
        input_vectors: torch.Tensor = self.in_embed.weight[input_words,:]                
    ...
    def forward_output(self, output_words: torch.Tensor) -> torch.Tensor:
        ...
        """
        # TODO
        output_vectors: torch.Tensor = self.out_embed.weight[output_words,:]
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        noise_words_reshapeado = noise_words.reshape((batch_size,n_samples))
        device: str = "cuda" if self.out_embed.weight.is_cuda else "cpu"
        noise_words_reshapeado: torch.Tensor = noise_words_reshapeado.to(device)

        # Reshape output vectors to size (batch_size, n_samples, n_embed)
        # TODO
        noise_vectors: torch.Tensor = self.out_embed(noise_words_reshapeado)
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        input_vectors = input_vectors.reshape((2,1,5))
        output_vectors = output_vectors.reshape((2,5,1))
        multiplicacion = torch.bmm(input_vectors,output_vectors)
        out_loss = torch.nn.functional.logsigmoid(multiplicacion).squeeze()

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_loss = torch.nn.functional.logsigmoid(-torch.bmm(input_vectors,noise_vectors.transpose(1,2))).squeeze()

        # Return the negative sum of the correct and noisy log-sigmoid losses, averaged over the batch
        # TODO
        total_loss = - (out_loss + noise_loss.sum(dim=1)).mean()
        return total_loss